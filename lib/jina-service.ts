...
export class JinaService {
  ...
  async parseWebPage(url: string): Promise<JinaReaderResponse> {
    try {
      ...
      const text = await response.data.data.content;
      ...
      const result = {
        ...
        data: {
          title:
            data.title ||
            this.extractTitleFromContent(data.content || text) ||
            "未知标题",
  ...
  async extractContentWithAI(
    ...
    try {
      ...
      const messages = [
        {
          role: "system",
          content: `你是一个专业的内容提取助手。用户会提供网页内容和特定的提取要求，请根据要求提取并整理内容。
          网页URL: ${url}
...
${userPrompt}`,
      ...
      if (!response.ok) {
        throw new Error(
          `DeepSearch API error: ${response.status} ${response.statusText}`
        );
      }
  ...
  async extractWebContent(
    request: WebContentExtractRequest
    ...
    try {
      ...
      if (request.userPrompt && request.extractMode === "custom") {
        const aiResult = await this.extractContentWithAI(
          content,
          request.userPrompt,
          url
        );
      ...
      // 3. 处理其他模式（full 和 summary）
      const result: ExtractedWebContent = {
        title: title || "未知标题",
        content: content,
        summary:
          request.extractMode === "summary"
            ? this.generateSummary(content)
            : undefined,
        tags: this.extractDefaultTags(content, title || ""),
  ...
  private extractDefaultTags(content: string, title: string): string[] {
    ...
    const foundTags = commonTags.filter(
      (tag) => text.includes(tag.toLowerCase()) || text.includes(tag)
    );
